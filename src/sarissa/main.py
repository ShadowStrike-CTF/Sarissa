...
def create_app(sessions_dir: Path | None = None, session_name: str = session.DEFAULT_SESSION) -> FastAPI:
    manager = session.SessionManager(sessions_dir)
    manager.open(session_name)
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument(
        "--session", default=session.DEFAULT_SESSION,
        help="named competition session (created if missing)",
    )
    parser.add_argument(
    ...
    try:
        ...
        app = create_app(session_name=args.session)
    except ValueError as exc:
        parser.error(str(exc))

    console = Console()
    console.print(f"[bold #922b21]SARISSA[/] — CTF dashboard  [dim]by ShadowStrike[/]")
    console.print(f"Session [bold]{args.session}[/] → {app.state.sessions.path_for(args.session)}")
